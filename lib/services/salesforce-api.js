...
class SalesforceApiService {
  ...
  async createContact(userId, contactData) {
    ...
    try {
      // Add the duplicate rule header to allow saving potential duplicates
  ...
  async createAccountWithContact(userId, data) {
    ...
    try {
      // Create Account or find existing one
      const accountResult = await this.createAccount(userId, accountData);

      // Add the Account ID to the Contact data
      const contactWithAccount = {
        ...contactData,
        AccountId: accountResult.id,
      };

      // Create Contact using the Account ID
