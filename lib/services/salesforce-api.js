...
class SalesforceApiService {
  ...
  async createAccountWithContact(userId, data) {
    // Generate a unique suffix to ensure uniqueness
    ...
    // Make contact fields unique to avoid duplicate detection

    // Always append timestamp to LastName (not just when empty)
    contactData.LastName = `${contactData.LastName || 'User'}-${timestamp}`;

    // Make email unique if it exists
    if (contactData.Email) {
      const emailParts = contactData.Email.split('@');
      if (emailParts.length === 2) {
        contactData.Email = `${emailParts[0]}+${timestamp}@${emailParts[1]}`;
      }
    }

    // Make phone unique if it exists
    if (contactData.Phone) {
      contactData.Phone = `${contactData.Phone}-${timestamp.slice(-4)}`;
    }
    ...
    try {
      ...
      return {
        ...
        success: true,
        isDuplicate: accountResult.isDuplicate || contactResult.isDuplicate,
