...
export const TemplateSchema = z
  ...
  .superRefine((data, ctx) => {
    // Only validate allowedUsers if template is not public
    if (!data.isPublic) {
      // Check if there are any allowed users when template is private
      if (data.allowedUsers.length === 0) {
        ...
        return; // Exit early if no users provided
      }

      // Validate email format only for private templates
